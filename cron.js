...
module.exports = { Post, User, Bot, Upvote, Account };
...
const postSubmission = async (reddit, submission) => {
    try {
        console.log("TRY");
        return reddit
            .getSubreddit(submission.sub)
            .submitSelfpost({title: submission.title, text: submission.text, flairId: submission.flair});
    } catch (err) {
        console.log(err)
    }
}
